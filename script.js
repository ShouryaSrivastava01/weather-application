...
let weather = {
    ...
    displayWeather: function (data) {
        ...
        document.querySelector(".description").innerHTML = `<span>${
            description[0].toUpperCase() + description.slice(1)
        }</span>`;
        document.getElementById(
            "temp"
        ).innerHTML = `<span>Temperature : ${temp}</span>`;
        document.querySelector(
            ".humidity"
        ).innerHTML = `<span>Humidity : ${humidity}</span>`;
        document.querySelector(".wind").innerHTML = `<span>Wind : ${(
            (speed * 18) /
            5
        ).toFixed(2)} km/h</span>`;
    },
    ...
    geoweather: function () {
        ...
        if (navigator)
            navigator.geolocation.getCurrentPosition((position) => {
                fetch(
                    `https://api.openweathermap.org/data/2.5/weather?lat=${position.coords.latitude}&lon=${position.coords.longitude}&appid=` +
                        this.apikey
                )
                    .then((response) => response.json())
                    .then((data) => this.fetchWeather(data.name));
            });
        else alert("no kr gsp");
        console.log(data);
    },
};
weather.geoweather();
